...
test('monitoring status remains independent of selected history and connection recovery', () => {
  const dom = loadFixtureDom('<div class="chart-widget-root"></div>');
  const { document } = dom.window;
  const panel = createStrategy27EventPanel(document, document.querySelector('.chart-widget-root'), panelOptions(2));
  ...
  assert.match(monitoring.textContent, /Symbol removed from monitoring/);
  document.querySelector('[data-event-id="old"]').click();
  panel.retainHistory();
  ...
  assert.match(connection.textContent, /Reconnecting/);
  panel.setOrdinaryConnection('stopped');
  assert.match(connection.textContent, /Stopped/);
  panel.clear();
  assert.equal(monitoring.dataset.state, 'historical');
  panel.destroy();
  dom.window.close();
