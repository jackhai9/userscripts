...
Alert markers use TradingView's drawing API. Every detected signal in the loaded window is rendered; there is no recent-signal truncation. More than 1,000 simultaneous signals is treated as an abnormal chart window and rejected before any partial marker mutation. The monitor then fails closed and clears the prior alert layer on its next pass instead of leaving an incomplete or stale historical view. Current live evidence shows that removing even a `disableSave` marker emits `drawing_event` and `saveChart`, so alert reconciliation pauses during every existing order-line drawing/save owner. Symbol changes, non-trading routes, hidden documents, and page teardown stop or clear the alert lifecycle.
