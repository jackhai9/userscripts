...
Do not validate orderbook precision apply by opening the dropdown manually first. The bug-prone path is the closed-dropdown path triggered by the script's Apply button. The current verified sequence is to dispatch `pointerdown`, `mousedown`, `pointerup`, `mouseup`, and `click` on `#futuresOrderbook .orderbook-tickSize` or its `.tick-content`, then wait until visible `.ob-ticksize-item` nodes exist before selecting the recommended value. A plain `element.click()` can leave the menu closed or options unrendered, which makes the script report a false "not found" state even though the option exists after a manual open.
