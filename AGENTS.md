...
- 真实委托量级压力测试按 `20 -> 40 -> 100` 逐档执行，不默认一次性挂到最大量级。由用户在当前 symbol 手动挂出远离盘口、不易成交的限价单，并在测试后手动撤单；Codex 只通过 CDP 记录每档的委托线变化数、完整保存次数、主线程阻塞和恢复耗时，不代替用户执行真实下单或撤单。每档结束后先确认无成交、无残留委托、页面状态已恢复，再进入下一档。
