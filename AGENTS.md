...
- Binance userscript 发布到 `main` 后，默认继续完成整条线上验证闭环：通过 Tampermonkey MCP 将生成产物写入现有脚本、反向读取核验版本和正文、强制刷新目标 Chrome 页面、用 raw CDP 核验实际加载 source，并执行与本次改动直接相关的现网页面路径。只要当前工具和安全边界允许，不要停在“已更新，请用户手测”；只有缺少登录态、MCP/CDP 连接失败、页面缺少必要测试状态、或最终动作受当前金融操作确认边界限制时才交接用户。
...
- Codex Chrome 的 `openTabs()`、`claimTab()` 和 page-level CDP 是不同故障层。`openTabs()` 正常而单个 `claimTab()` / `tabs.get()` 挂起时，先按 stale browser-session ownership 处理，不要归因于目标网站或代理节点：同一 `node_repl` kernel 只初始化一次 browser runtime 并复用 binding；claim 超时的结果视为 unknown，禁止重复 claim、重复建 client/session 或继续使用旧 handle。能在同一登录态副本页完成的只读诊断优先用一个 agent-created 临时 tab；只有所有新 tab 也失败时才转入 helper/proxy/CDP 排查。browser 验证未完成时不要重置 kernel，任务结束时不要为普通诊断 tab 设置 handoff/deliverable，让 turn cleanup 释放控制权。详细本机 SOP 见 `~/.dotfiles/knowledge/machines/lizhenhai-macbook-pro-14/LOCAL_PROXY_REFERENCE.md` 的“Codex Chrome `claimTab` 结果丢失与 stale session 占用”。
...
- Binance 现网页面是用户与 Codex 共用的交易界面，CDP 验证期间用户可能同时手动交易。订单数量、方向、价格或面板状态发生变化时，必须先按测试动作时间窗、订单创建时间、symbol、方向、数量和 userscript 状态迁移建立因果归属；不能仅凭验证前后数量变化，把用户并行创建的订单判断为撤单残留、脚本回归或异常并长时间偏离核心验证。无法归属的订单默认不纳入测试结论和清理范围，不得擅自修改或撤销；只有它实质阻断本次验证且现有证据无法隔离时，才简短向用户确认。
- 用户的长期 CDP 实测偏好是：在当前 symbol 使用远离盘口、不易成交的限价单制造真实委托线，完成采样后撤销测试挂单；后续不要再要求用户重复解释这套造数方法。默认新挂单量级依次为 `20` / `40` / `120`；每轮前读取当前 symbol 已有未成交委托数，确保“已有 + 本轮新增”总数不超过 `199`，余量不足时缩减本轮新增数，不把 Binance 当前 `200` 上限当作实测目标。测试前仍必须从当前 symbol 的实时 `/fapi/v1/exchangeInfo` 读取 `MAX_NUM_ORDERS.limit`；若该上限低于 `200`，以实时上限减一为总数上限。实际执行责任以当前运行环境的金融操作安全边界为准；如需用户亲自提交或撤销，Codex 应完成其余准备和 CDP 监测，只在最终金融动作处交接。每轮结束后先确认无成交、无残留测试委托、页面状态已恢复，再进入下一量级；用户并行产生的订单不属于测试残留。
- 用户已经明确授权将上述远价真实挂单和撤单用作发布后的 CDP 验证造数手段；不要因为“需要真实订单”而提前停止验证。仍须遵守当前运行环境的金融操作确认边界，并优先选择不产生真实成交或资金变化、但足以覆盖本次契约的最小验证路径；例如只验证撤单弹窗取消/关闭语义时，复用现有挂单、等待目标时长后点击“取消”，不需要先制造更多订单或点击“确认”。
