...
- 真实委托量级压力测试应根据验证目标从小到大逐步加压，`20` / `40` / `100` 只是可选样本，不是固定档位。测试前必须从当前 symbol 的实时 `/fapi/v1/exchangeInfo` 读取 `MAX_NUM_ORDERS.limit`，并为账户已有的普通单和条件单留出余量，不把 `200` 等当前观测值硬编码为永久上限。由用户在当前 symbol 手动挂出远离盘口、不易成交的限价单，并在测试后手动撤单；Codex 只通过 CDP 记录委托线变化数、完整保存次数、主线程阻塞和恢复耗时，不代替用户执行真实下单或撤单。每轮结束后先确认无成交、无残留委托、页面状态已恢复，再决定是否扩大量级。
