...
(() => {
  ...
  (function() {
    ...
    function holderDisplayMetric(detail, symbol) {
      ...
      const holders = holdersFromPageMetric !== null ? holdersFromPageMetric : detail.holders && typeof detail.holders === "object" ? numberOrNull(detail.holders.holderCount || detail.holders.total || detail.holders.count) : null;
      return {
        label: "持有者",
    ...
    function formatProfileScore(value) {
      if (value && typeof value === "object") {
        const percentage2 = numberOrNull(value.percentage);
        return percentage2 !== null ? percentage2.toFixed(0) + "%" : "--";
      }
    ...
    function buildRows(detail) {
      ...
      const volumeToMarketCap = numberOrNull(stats.turnover) !== null ? numberOrNull(stats.turnover) * 100 : null;
      const liquidityToMarketCap = numberOrNull(stats.liquidityMcapRatio) !== null ? numberOrNull(stats.liquidityMcapRatio) * 100 : null;
    ...
    function ensurePanel() {
      ...
      panel.innerHTML = [
        ...
        "padding:8px 10px;cursor:move;background:#fafafa;border-bottom:1px solid ",
        C.border,
        ";",
        '">',
        ...
        '-symbol" style="color:',
        C.sub,
        ';font-size:13px;overflow:hidden;text-overflow:ellipsis;"></span>',
        "</div>",
        '<div style="display:flex;gap:2px;flex:0 0 auto;">',
        '<button id="',
        PANEL_ID,
        '-refresh" title="刷新" style="',
        "background:none;border:none;cursor:pointer;font-size:14px;color:",
        C.sub,
        ...
        "background:none;border:none;cursor:pointer;font-size:15px;color:",
        C.sub,
        ";padding:0 3px;line-height:1;",
        '">',
        collapsed ? "&#9633;" : "&#95;",
        ...
        '<div id="',
        PANEL_ID,
        '-rows" style="padding:8px 10px;"></div>',
        '<div id="',
        PANEL_ID,
        '-footer" style="padding:6px 10px;color:',
        C.sub,
    ...
    function renderError(symbol, message) {
      ...
      if (rowsEl) {
        lastRowsHtml = [
          '<div style="color:',
          C.short,
          ';font-weight:600;padding:4px 0;">读取失败</div>',
          '<div style="color:',
          C.sub,
    ...
    function renderData(symbol, data) {
      ...
      if (rowsEl) {
        lastRowsHtml = data.rows.map(function(row) {
          const change = row.change && row.change !== "--" ? '<span style="margin-left:6px;color:' + (row.change.startsWith("-") ? C.short : C.long) + ';">' + escapeHtml(row.change) + "</span>" : "";
          const cardStyle = row.highlight ? "background:linear-gradient(180deg, rgba(56,97,251,.055), rgba(22,199,132,.045));box-shadow:inset 0 0 0 1.5px rgba(56,97,251,.62);border-radius:6px;padding:5px 6px;margin:2px -6px;" : "padding:4px 0;";
          return [
      ...
      if (footerEl) {
        ...
        footerEl.innerHTML = [
          ...
          sourceLabel,
          "</a>",
          '<span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:right;">CMC ',
          cmcClock,
          " / 拉取 ",
