...
(() => {
  ...
  function getOrderbookPrecisionDecadeTarget(options, current, direction) {
    ...
    const target = direction === "INCREASE" ? multiplyDecimalByInt(normalizedCurrent, 10) : multiplyDecimalByRatio(normalizedCurrent, 1, 10);
    if (!target) return null;
    const available = new Set((options || []).map((value) => normalizeDecimalString(value)).filter((value) => value && isPositiveDecimalString(value)));
  ...
  (function() {
    ...
    function refreshOrderbookPrecisionRecommendation(panel = document.getElementById(PANEL_ID)) {
      ...
      const recommendationHtml = [
        ...
        '<div style="display:grid;grid-template-columns:36px 32px 60px 32px;align-items:center;justify-content:start;gap:6px;height:32px;overflow:hidden;">',
        ...
        `<span title="当前缩放 ${currentText}" style="min-width:0;height:32px;border:1px solid #d5d9e2;border-radius:6px;background:#ffffff;text-align:center;font-size:15px;font-weight:600;line-height:30px;color:#1e2329;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${currentText}</span>`,
