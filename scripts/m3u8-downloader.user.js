...
(() => {
  ...
  var init_constants = __esm({
    "src/m3u8-downloader/constants.js"() {
      ...
      BROOKS_MEDIA_INDEX_MESSAGE_TYPE = "jh-userscripts:brooks-media-index-record";
  ...
  function buildCaptionUrlFromM3u82(url, captionFile) {
    ...
    const baseIndex = pathParts[videoIndex] === "video.m3u8" && videoIndex > 0 && /^\d+x\d+$/.test(pathParts[videoIndex - 1]) ? videoIndex - 1 : videoIndex;
  ...
  function getYtDlpOutputName(title) {
    return title.replace(/\s*\|\s*Brooks Trading Course\s*$/i, "").replace(/[/:*?"<>|]/g, "_").trim() + ".%(ext)s";
  }
  ...
  // src/m3u8-downloader/brooks-pages.js
  function normalizeBrooksTitle(title) {
    return (title || "").replace(/\s*\|\s*Brooks Trading Course\s*$/i, "").replace(/^BTC PAF/i, "Video").trim();
  }
  function isBrooksHost(hostname) {
    return hostname === "brookstradingcourse.com" || hostname.endsWith(".brookstradingcourse.com");
  }
  function isBrooksCourseIndexPage(locationObj = location) {
  ...
  function getBrooksCourseVideoLinks(root) {
    ...
    }).filter((url) => url && isBrooksHost(url.hostname) && isBrooksMediaPageUrl(url)).map((url) => url.href).filter((href) => {
      if (seen.has(href)) {
        return false;
      }
      seen.add(href);
  ...
  var init_brooks_pages = __esm({
    "src/m3u8-downloader/brooks-pages.js"() {
    }
  });

  // src/m3u8-downloader/brooks-record.js
  function buildBrooksMediaIndexRecord(options) {
    ...
    return {
      ...
      cn: buildCaptionUrlFromM3u82(options.m3u8Url, "CN.vtt"),
      en: buildCaptionUrlFromM3u82(options.m3u8Url, "EN.vtt"),
      index: options.index
    };
  }
  var init_brooks_record = __esm({
    "src/m3u8-downloader/brooks-record.js"() {
      init_media_url();
    }
  });

  // src/m3u8-downloader/brooks-status.js
  ...
  function getBrooksMediaExportElapsedMs(state, now) {
    ...
    const baseElapsedMs = typeof state.activeElapsedMs === "number" && Number.isFinite(state.activeElapsedMs) ? Math.max(0, state.activeElapsedMs) : null;
    ...
    const activeRunMs = state.running && activeRunStartedAt !== null ? Math.max(0, fallbackNow - activeRunStartedAt) : 0;
  ...
  function formatBrooksMediaExportStatus(options) {
    ...
    if (pending && pending.url) {
      ...
      if (pending.startedAt) {
        const elapsedSeconds = Math.max(0, Math.floor(((options.now || Date.now()) - pending.startedAt) / 1e3));
  ...
  function buildBrooksMediaExportPayload(state, exportedAt) {
    ...
    const missingIndexes = links.map((url, index) => index).filter((index) => !completedIndexes.has(index));
    const done = records.length + failures.length;
    return {
      exportedAt,
      ...
      elapsedMs,
      elapsedSeconds: elapsedMs === null ? null : Math.floor(elapsedMs / 1e3),
  ...
  function createBrooksMediaExporter({ originXHR: originXHR2, downloadWithA: downloadWithA2, getTitle: getTitle2 }) {
    ...
    function saveBrooksMediaExportState() {
      ...
      brooksMediaExportState.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
    ...
    function isCurrentBrooksMediaExportPending(index, url) {
      return !!(brooksMediaExportState && brooksMediaExportState.running && !brooksMediaExportState.stopped && brooksMediaExportPending && brooksMediaExportPending.index === index && isSameBrooksVideoPage(brooksMediaExportPending.url, url));
    }
    function fetchBrooksMediaExportPageInfo(url, onSuccess, onFailure) {
      const xhr = new originXHR2();
    ...
    function retryFailedBrooksMediaExport() {
      ...
      const retryIndexes = (brooksMediaExportState.failures || []).map((failure) => failure.index).filter((index) => typeof index === "number" && brooksMediaExportState.links[index]);
  ...
  var init_brooks_exporter = __esm({
    "src/m3u8-downloader/brooks-exporter.js"() {
      init_constants();
      init_brooks_pages();
      init_brooks_record();
  ...
  var require_index_user = __commonJS({
    "src/m3u8-downloader/index.user.js"(exports, module) {
      ...
      (function() {
        ...
        var mp4Objs = [];
        var mp4UrlSet = /* @__PURE__ */ new Set();
        var m3u8UrlSet = /* @__PURE__ */ new Set();
        var originXHR = window.XMLHttpRequest;
        var windowOpen = window.open;
        var brooksMediaExporter = createBrooksMediaExporter({ originXHR, downloadWithA, getTitle });
        ...
        function checkM3u8Url(url) {
          ajax({
            url,
            success: (fileStr2) => {
              if (/(png|image|ts|jpg|mp4|jpeg|EXTINF)/.test(fileStr2)) {
        ...
        function showM3u8Controls(url, referer) {
          ...
          brooksMediaExporter.notifyMediaIndexDetected(url, m3u8Referer);
          appendDom();
          const m3u8Jump2 = document.getElementById("m3u8-jump");
          ...
          m3u8Jump2.style.display = isExternalDownloaderBlocked(url) ? "none" : "block";
        }
    ...
    `;
        ...
        function scanVideo(video) {
          const sourceUrls = [video.currentSrc, video.src].concat(Array.from(video.querySelectorAll("source")).map((source) => source.src)).filter(Boolean);
  ...
  " id="m3u8-close">
    ...
    `;
          ...
          m3u8Append.addEventListener("click", function() {
            ...
            ajax({
              url: "https://blog.luckly-mjw.cn/tool-show/m3u8-downloader/index.html?t=" + (/* @__PURE__ */ new Date()).getTime(),
              success: (fileStr) => {
        ...
        function switchMp4Download() {
          ...
          $section2.id = "mp4-download-dom";
          $section2.style.position = "fixed";
          $section2.style.zIndex = "9999";
          $section2.style.top = "20px";
          $section2.style.right = "20px";
          $section2.style.textAlign = "center";
          mp4Objs.forEach((obj) => {
            var $mp42 = document.createElement("div");
            $mp42.innerHTML = obj.fileName;
            $mp42.title = obj.url;
            $mp42.style = `
      margin-top: 4px;
      padding: 3px 4px ;
      ...
      background-color: #3D8AC7;
      `;
            $mp42.addEventListener("click", () => {
              downloadWithA(obj.url, obj.fileName);
            });
            $section2.appendChild($mp42);
          });
