...
const TAB_LABELS = { upgrade_https: 'HTTPS Upgrade', delete: 'Delete', review: 'Review', keep: 'Keep' };
...
async function initReview() {
  if (!batchId) {
    $('h1').textContent = 'No batch ID. Please import from popup.';
    return;
  }
  ...
  if (!tsvRows || tsvRows.length === 0) {
    $('h1').textContent = 'No TSV data. Please import from popup.';
    return;
  }

  $('h1').textContent = `Matching ${tsvRows.length} URLs with Chrome bookmarks…`;
...
async function initRestore() {
  ...
  if (!log || log.length === 0) {
    $('h1').textContent = 'No restore log. Please import from popup.';
    return;
  }

  await chrome.storage.local.remove(storageKey).catch(() => {});

  // Validate restore log entries
  const validEntries = [];
  const invalidCount = { missing: 0, badAction: 0 };
  for (const entry of log) {
    if (entry.status !== 'ok') continue;
    if (!entry.id || !entry.oldUrl || !entry.action) {
      invalidCount.missing++;
      continue;
    }
    if (!VALID_RESTORE_ACTIONS.has(entry.action)) {
      invalidCount.badAction++;
      continue;
    }
    if (entry.action === 'upgrade' && !entry.newUrl) {
      invalidCount.missing++;
      continue;
    }
    if (entry.action === 'delete' && !entry.parentId) {
      invalidCount.missing++;
      continue;
    }
    ...
    try {
      ...
      if (oldParsed.protocol !== 'http:' && oldParsed.protocol !== 'https:') {
        invalidCount.badAction++;
        continue;
      }
      if (entry.newUrl) {
        const newParsed = new URL(entry.newUrl);
        if (newParsed.protocol !== 'http:' && newParsed.protocol !== 'https:') {
          invalidCount.badAction++;
          continue;
        }
      }
  ...
  let info = `Restore mode: ${validEntries.length} valid operations to undo.`;
  if (invalidCount.missing > 0 || invalidCount.badAction > 0) {
    info += ` Skipped: ${invalidCount.missing} with missing fields, ${invalidCount.badAction} with unknown action.`;
  }
  restoreBox.textContent = info;

  if (validEntries.length === 0) {
    $('h1').textContent = 'No valid restore entries found.';
    return;
  }

  $('h1').textContent = 'Bookmark Batch Fixer — Restore';
  ...
  for (const entry of validEntries) {
    ...
    tdUrl.title = entry.oldUrl || '';
    tdUrl.textContent = entry.oldUrl || '';

    const tdAction = document.createElement('td');
    tdAction.className = 'folder-cell';
    ...
    span.textContent = `${entry.action} → undo`;
  ...
  $('#btn-exec').textContent = 'Restore Selected';
...
async function executeRestore(entries) {
  ...
  // Global lock: refuse to start if any other execution/restore is active
  ...
  if (lockCheck.pending_changelog || lockCheck.pending_restore_journal || lockCheck.executing_batch) {
    alert('Another execution or restore is still in progress (or was interrupted). Please recover or discard it from the popup first.');
    return;
  }
  ...
  summary.textContent = 'Restoring… Do not close this tab.';
  ...
  const restoreJournal = ops.map((entry) => ({
    ...entry,
    restoreStatus: 'pending',
    restoreError: null,
  }));

  // Fail-stop on initial journal write; also set executing flag
  try {
    await chrome.storage.local.set({
      pending_restore_journal: restoreJournal,
      executing_batch: { batchId: batchId || 'restore', since: Date.now() },
    });
  } catch (e) {
    summary.textContent = `Cannot save restore journal: ${e.message}. Aborting.`;
    ...
    try {
      if (jEntry.action === 'upgrade') {
        ...
        if (!current || current.length === 0) {
          jEntry.restoreStatus = 'skipped';
          jEntry.restoreError = 'bookmark no longer exists';
          done++;
          bar.style.width = `${(done / ops.length) * 100}%`;
          journalStale = !(await persistJournal('pending_restore_journal', restoreJournal)) || journalStale;
          continue;
        }
        if (current[0].url !== jEntry.newUrl) {
          jEntry.restoreStatus = 'skipped';
          jEntry.restoreError = `URL already changed: expected ${jEntry.newUrl}, found ${current[0].url}`;
          done++;
          bar.style.width = `${(done / ops.length) * 100}%`;
          journalStale = !(await persistJournal('pending_restore_journal', restoreJournal)) || journalStale;
          continue;
        ...
        if (alreadyRestored) {
          jEntry.restoreStatus = 'skipped';
          jEntry.restoreError = 'bookmark already exists at target location';
          done++;
          bar.style.width = `${(done / ops.length) * 100}%`;
          journalStale = !(await persistJournal('pending_restore_journal', restoreJournal)) || journalStale;
          continue;
      ...
      jEntry.restoreStatus = 'ok';
      ok++;
    } catch (e) {
      jEntry.restoreStatus = 'error';
      jEntry.restoreError = e.message;
      fail++;
  ...
  // Clear both journal and executing flag together
  await chrome.storage.local.remove(['pending_restore_journal', 'executing_batch']).catch(() => {});

  let msg = `Restore complete: ${ok} succeeded, ${fail} failed, ${ops.length - ok - fail} skipped.`;
  if (journalStale) msg += ' WARNING: Some journal writes failed — if this was interrupted, recovery data may be incomplete.';
...
function buildTabs() {
  ...
  for (const key of TAB_ORDER) {
    ...
    btn.className = 'tab';
    btn.dataset.tab = key;
    btn.innerHTML = `${TAB_LABELS[key]} <span class="badge">${counts[key]}</span>`;
...
function renderTable() {
  ...
  for (const item of items) {
    ...
    const syncLabel = item.syncing === true ? 'sync' : item.syncing === false ? 'local' : 'unknown';
...
function updateSelectedCount() {
  ...
  $('#selected-count').textContent = `${count}/${total} selected`;
...
async function executeSelected() {
  ...
  if (selected.length === 0) {
    alert('No actionable items selected. Only HTTPS Upgrade, Delete, and Review items can be executed.');
    return;
  }
  ...
  const bar = $('#progress-bar');
  const summary = $('#summary');
  summary.textContent = 'Executing… Do not close this tab.';
  ...
  const changeLog = selected.map((item) => {
    ...
    // Validate HTTPS upgrade target URL
    if (action === 'upgrade' && newUrl) {
      try {
        const parsed = new URL(newUrl);
        if (parsed.protocol !== 'https:') {
          return {
            action, id: item.bookmark.id, parentId: item.bookmark.parentId,
            index: item.bookmark.index, title: item.bookmark.title,
            oldUrl: item.bookmark.url, newUrl,
            status: 'skipped', error: `target URL is not https: ${parsed.protocol}`,
          };
        }
      } catch {
        return {
          ...
          index: item.bookmark.index, title: item.bookmark.title,
          oldUrl: item.bookmark.url, newUrl,
          status: 'skipped', error: `invalid target URL: ${newUrl}`,
        };
      }
    }
  ...
  // Fail-stop on initial write; set executing flag with timestamp
  try {
    await chrome.storage.local.set({
      pending_changelog: changeLog,
      executing_batch: { batchId: batchId || 'exec', since: Date.now() },
    });
  } catch (e) {
    summary.textContent = `Cannot save change log: ${e.message}. Aborting — no bookmarks were modified.`;
    summary.className = 'summary error';
    btn.disabled = false;
    executing = false;
    return;
  ...
  for (const entry of changeLog) {
    if (entry.status !== 'pending') {
      ...
      bar.style.width = `${(done / changeLog.length) * 100}%`;
      continue;
    }

    // TOCTOU — verify bookmark state before mutation; record actual metadata
    try {
      const current = await chrome.bookmarks.get(entry.id);
      if (!current || current.length === 0) {
        entry.status = 'skipped';
        entry.error = 'bookmark no longer exists';
        done++;
        bar.style.width = `${(done / changeLog.length) * 100}%`;
      ...
      if (bm.url !== entry.oldUrl) {
        entry.status = 'skipped';
        entry.error = `URL changed since scan: expected ${entry.oldUrl}, found ${bm.url}`;
        done++;
        bar.style.width = `${(done / changeLog.length) * 100}%`;
        journalStale = !(await persistJournal('pending_changelog', changeLog)) || journalStale;
        continue;
      }
      // Record actual parentId/index/title at execution time for accurate restore
      entry.parentId = bm.parentId;
      entry.index = bm.index;
      entry.title = bm.title;
    } catch (e) {
      entry.status = 'skipped';
      entry.error = `pre-check failed: ${e.message}`;
      done++;
      bar.style.width = `${(done / changeLog.length) * 100}%`;
      journalStale = !(await persistJournal('pending_changelog', changeLog)) || journalStale;
      continue;
    ...
    try {
      if (entry.action === 'upgrade' && entry.newUrl) {
        await chrome.bookmarks.update(entry.id, { url: entry.newUrl });
        entry.status = 'ok';
      } else if (entry.action === 'delete') {
        await chrome.bookmarks.remove(entry.id);
        entry.status = 'ok';
      } else {
        entry.status = 'skipped';
  ...
  // Wait for download BEFORE clearing executing flag and journal
  summary.textContent = 'Saving change log… Do not close this tab.';
  ...
  if (downloadSaved) {
    // Clear journal AND executing flag together, only after confirmed download
    await chrome.storage.local.remove(['pending_changelog', 'executing_batch']).catch(() => {});
  } else {
    // Download failed — keep journal but clear executing flag so popup can help recover
    await chrome.storage.local.remove('executing_batch').catch(() => {});
    let msg = `Done: ${ok} succeeded, ${fail} failed. WARNING: Download may have failed — change log kept in extension storage. Use popup to recover.`;
    if (journalStale) msg += ' ALSO: Some journal writes failed during execution.';
    summary.textContent = msg;
    summary.className = 'summary error';
    buildTabs();
    renderTable();
  ...
  let msg = `Done: ${ok} succeeded, ${fail} failed, ${skipped} skipped. Change log saved.`;
  if (journalStale) msg += ' Note: Some journal writes failed during execution — recovery data may be incomplete if this was interrupted.';
  summary.textContent = msg;
  summary.className = (fail > 0 || journalStale) ? 'summary error' : 'summary';
...
// Journal persistence — returns true on success, false on failure
// Also updates executing_batch.since as heartbeat to prevent stale detection
async function persistJournal(key, data) {
  try {
    await chrome.storage.local.set({
      [key]: data,
      executing_batch: { batchId: batchId || 'active', since: Date.now() },
    });
    return true;
  } catch (e) {
...
// Wait for chrome.downloads to reach 'complete' state, with timeout
